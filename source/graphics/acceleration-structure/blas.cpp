	...
	vk::AccelerationStructureBuildRangeInfoKHR* rangeInfos, ID<Buffer>* syncBuffers, BuildFlagsAS flags)
{
	for (uint32 i = 0; i < geometryCount; i++)
	{
		...
		GARDEN_ASSERT_MSG(vertexBufferView->getDeviceAddress(), 
		...
		GARDEN_ASSERT_MSG(indexBufferView->getDeviceAddress(), 
	...
	vk::AccelerationStructureBuildRangeInfoKHR* rangeInfos, ID<Buffer>* syncBuffers, BuildFlagsAS flags)
{
	for (uint32 i = 0; i < geometryCount; i++)
	{
		...
		GARDEN_ASSERT_MSG(aabbBufferView->getDeviceAddress(), 
			"AABB buffer [" + aabbBufferView->getDebugName() + "] is not ready");
