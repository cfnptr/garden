...
public:
	...
	/**
	 * @brief Framebuffer blending functions.
	 * 
	 * @details
	...
	/**
	 * @brief Stencil comparison functions.
	 *
	 * @details 
	 * It acts as a set of instructions for the fixed-function pipeline, determining whether to preserve the current 
	 * value, reset it to zero, replace it with a reference, or modify it via incrementing, decrementing, or bitwise 
	 * inversion. These operations are fundamental for implementing complex rendering techniques such as shadow volumes, 
	 * non-photorealistic outlines, and masking specific regions of the frame for multi-pass rendering.
	 */
	enum class StencilOp : uint8
	{
		Keep,      /**< Keeps the current value. */
		Zero,      /**< Sets the value to 0x00. */
		Replace,   /**< Sets the value to reference. */
		IncrClamp, /**< Increments the current value and clamps to the maximum representable unsigned value. */
		DecrClamp, /**< Decrements the current value and clamps to 0. */
		Invert,    /**< Bitwise inverts the current value. */
		IncrWrap,  /**< Increments the current value and wraps to 0 when the max value would have been exceeded. */
		DecrWrap,  /**< Decrements the current value and wraps to the maximum possible value when the value would go below 0. */
		Count      /**< Stencil operation count. */
	};
	/**
	...
	struct BlendState final
	{
		...
		BlendOp colorOperation = BlendOp::Add;                      /**< Color components (R, G, B) blending operation. */
	...
	/**
	 * @brief Stencil test operations state.
	 *
	 * @details
	 * Fixed-function configuration in the graphics pipeline that controls the Stencil Test, a per-fragment operation 
	 * used to discard or keep pixels based on a comparison between a reference value and the contents of a dedicated 
	 * stencil buffer. Often described as a masking stage, it allows to define complex drawing regions by testing if a 
	 * fragment's stencil value satisfies a specific condition. Enabling advanced visual effects such as mirrors, 
	 * portals, shadow volumes, UI masking and more.
	 */
	...
	struct State final
	{
		...
		float2 depthBounds = float2(0.0f, 1.0f);           /**< Minimum and maximum depth bounds. */
		StencilState frontFaceStencil = {};                /**< Front-facing polygons stencil state. */
...
{
	if (cullFace == "none") return GraphicsPipeline::CullFace::None;
	if (cullFace == "front") return GraphicsPipeline::CullFace::Front;
	if (cullFace == "back") return GraphicsPipeline::CullFace::Back;
	if (cullFace == "frontBack") return GraphicsPipeline::CullFace::FrontBack;
 ...
 */
static GraphicsPipeline::BlendOp toBlendOp(string_view blendOperation)
{
	if (blendOperation == "add") return GraphicsPipeline::BlendOp::Add;
	if (blendOperation == "subtract") return GraphicsPipeline::BlendOp::Subtract;
	if (blendOperation == "revSubtract") return GraphicsPipeline::BlendOp::RevSubtract;
	if (blendOperation == "minimum") return GraphicsPipeline::BlendOp::Minimum;
	if (blendOperation == "maximum") return GraphicsPipeline::BlendOp::Maximum;
